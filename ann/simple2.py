...
class data:
    ...
    def add_column(self,v):
        from numpy import full,column_stack
        a=full(v,len(self.x))
        self.x2=column_stack((self.x,a))
...
def main():
    ...
    x_stacked=column_stack((x,x_))

    x_stacked=array(x_stacked,dtype='float32')
    #y=array(y,dtype='float32')

    feed_x=reshape(x_stacked,(-1,2))
    ...
    xo1_stacked=column_stack((xo1,xo1_))

    xo1=array(xo1,dtype='float32')
    xo1_stacked=array(xo1_stacked,dtype='float32')

    yo1=array(yo1,dtype='float32')

    feed_x_true=reshape(xo1_stacked,(-1,2))
    feed_y_true=reshape(yo1,(-1,1))
    ...
    plot(xo1,y_predicted,"--",alpha=0.5,label="predicted")
    xlabel(r"$x$")
    ylabel(r"$y$")
