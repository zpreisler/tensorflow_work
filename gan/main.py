...
def generator(Z,reuse=False):
    with tf.variable_scope("GAN/Generator",reuse=reuse):
        d1=tf.layers.dense(inputs=Z,units=24,activation=tf.nn.leaky_relu,name="g1")
        d2=tf.layers.dense(inputs=d1,units=16,activation=tf.nn.leaky_relu,name="g2")
        output_layer=tf.layers.dense(d1,units=2,name="g_out")

    return output_layer
        
def discriminator(X,reuse=False):
    with tf.variable_scope("GAN/Discriminator",reuse=reuse):
        d1=tf.layers.dense(inputs=X,units=24,activation=tf.nn.leaky_relu,name="d1")
        d2=tf.layers.dense(inputs=d1,units=16,activation=tf.nn.leaky_relu,name="d2")
...
def main(argv):
    ...
    generator_vars=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,scope="GAN/Generator")
    discriminator_vars=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,scope="GAN/Discriminator")

    generator_optimizer=tf.train.AdamOptimizer(learning_rate=1e-2)
    discriminator_optimizer=tf.train.AdamOptimizer(learning_rate=1e-2)
